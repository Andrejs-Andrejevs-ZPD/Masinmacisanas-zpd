...
trimmed = os.path.join('C:\machine_learning\Voice_debate', 'zpd_1.wav')
y,sr = librosa.load(trimmed)
...
ax.set(title='Mel frekvences spektogramma')
#   plt.xlim(60,120)
plt.show()
...
#   read CSV file
data_path = os.path.join('C:\machine_learning\Voice_debate', 'zpd_1.csv')
...
pca_data['speaker']=CLASS[:len(pca_data)]
pca_data['Time']=audio_time
pca_data.head()

#plt.grid(True)
sns.pairplot(data =pca_data, hue='speaker', plot_kws={'s':1}, palette='plasma')
plt.show()

mi(X=pca_data[['Time',0,1,2,3]],y=pca_data['speaker'])
time_series_data = pca_data[['Time',2,'speaker']]  

#sns.scatterplot(x='Time', y='X', hue='speaker', data=time_series_data, palette='plasma')  #    check workability
#plt.grid(True)
#plt.show()

#   Label encoding
label_encoder = preprocessing.LabelEncoder()
time_series_data['speaker']= label_encoder.fit_transform(time_series_data['speaker'])

encod_dic = {0:'Autors',2:'Runātājs 3',1:'Runātājs 2'}
...
test_data['Autors']=pred_test['Autors']
test_data['Runātājs 2']=pred_test['Runātājs 2']
test_data['Runātājs 3']=pred_test['Runātājs 3']
...
sns.scatterplot(test_data.Time,test_data,hue=test_data.target,palette='plasma')
plt.grid(True)
plt.show()
